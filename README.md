...
If you are new to the project and want the plain-English version first, start with `PROJECT_OVERVIEW.md`.

## Scope

This repository currently implements the LearnX core engine and AI layer as a plain Java 17 Maven project. It is designed to be embedded later inside a Spring Boot backend and consumed by a web-only frontend with an app-grade user experience.

A first frontend slice now lives under `web/` as a Next.js + TypeScript + Tailwind scaffold with public landing, auth, and protected app shells.
...
- Tavily grounding with composite result merging
...
The new web app scaffold is under `web/`.

Node.js `20.9+` is required for the frontend workspace.
